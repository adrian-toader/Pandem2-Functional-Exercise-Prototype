...
export class UserRoleHelper {
    ...
    if (
      ...
      option.requires.forEach((requiredPermission: PERMISSION) => {
        if (
          ...
          requiredPermissionTranslations.push(optionsMap[requiredPermission].option.label);
        }
      });
      ...
      //     )
      // );
      response.label = 'LNG_ROLE_AVAILABLE_PERMISSIONS_GROUP_LABEL_MESSAGE';
      // tooltip
  ...
  private static displayRequiredByPopup(
    ...
    // labels
    const requiredByLabel: string = 'LNG_ROLE_AVAILABLE_PERMISSIONS_REQUIRED_BY_PERMISSIONS_LABEL';
    const requiredLabel: string = 'LNG_ROLE_AVAILABLE_PERMISSIONS_REQUIRED_PERMISSIONS_LABEL';
    ...
    let labels: string[] = requiredByList.map((permission): string => {
      return `<div>${data.optionsMap[permission] ?
        (data.optionsMap[permission].option as IPermissionChildModel).label : (
          data.groupsMap[permission] ?
            (data.groupsMap[permission] as PermissionModel).groupLabel :
            permission
        )}</div>`;
    });
