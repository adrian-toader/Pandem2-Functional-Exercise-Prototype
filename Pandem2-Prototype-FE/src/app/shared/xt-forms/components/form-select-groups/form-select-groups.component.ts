    ...
    this.groups.forEach((group) => {
      ...
      (group[this.groupOptionsKey] || []).forEach((option) => {
        ...
        if (this.groupOptionFormatMethod) {
          ...
          // label
          if (this.groupOptionLabelKey) {
            this.labelTranslations[option[this.groupOptionLabelKey]] = option[this.groupOptionLabelKey];
          }

          // tooltip
          if (this.groupOptionTooltipKey) {
            this.tooltipTranslations[option[this.groupOptionTooltipKey]] = option[this.groupOptionTooltipKey];
          }
        }
      });
    });
      ...
      (this.value || []).forEach((selectedOptionValue: string) => {
        ...
        labels.push(
          this.groupsMap[selectedOptionValue] ?
            `${this.groupsMap[selectedOptionValue][this.groupLabelKey]}` :
            `${!this.optionsMap[selectedOptionValue] ? '' : this.optionsMap[selectedOptionValue].option[this.groupOptionLabelKey]}`
        );
      });
        ...
        (this.value || []).forEach((selectedValue: string) => {
          ...
          if (
            ...
            this.partialOptions[value] = this.partialOptions[value] +
                          ...
                          this.optionsMap[selectedValue].option[this.groupOptionLabelKey];
