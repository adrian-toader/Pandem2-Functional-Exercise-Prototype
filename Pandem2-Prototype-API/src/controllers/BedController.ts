...
import { GroupManager } from '../components/beds/GroupManager';
import { IDailyBedsFilter, ILocationsBedsDateIntervalFilter } from '../interfaces/beds';

/**
 * Generate dummy data for beds occupancy
 * @param request
 * @param reply
 */
export const generateDummyData = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const generator = new BedGenerator(payload.location, payload.generate_for_sublocations);
    ...
    }, 'Error generating dummy data for beds');
...
/**
 * Get all daily beds statistics for selected location
 * @param request
 * @param reply
 */
export const getDaily = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const groupManager = new GroupManager(request.query as IDailyBedsFilter);
...
export const getLocationsDaily = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    request.log.error({
      ...
      stack: err.stack,
      params: request.params
    }, 'Error retrieving locations daily beds');
...
export const getLocationsDateInterval = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const groupManager = new GroupManager(request.query as IDailyBedsFilter);
    const data = await groupManager.getLocationsDateInterval(request.query as ILocationsBedsDateIntervalFilter);
